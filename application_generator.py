...
class JobApplicationBuilder:
    ...
    @staticmethod
    def save_latex_as_pdf(tex_file_path: str, destination_path: str = None, latex_engine: str = None):
        """
        Compiles a LaTeX `.tex` file into a PDF using a specified LaTeX engine and saves it to a specified directory.

        This method runs the appropriate LaTeX engine (`pdflatex`, `xelatex`, or `lualatex`) on the provided `.tex` file.
